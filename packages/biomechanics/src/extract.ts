...
import type { FrameFeatures, Landmark, PoseFrame } from '@ai-pushup-coach/types';
import {
  L_ANKLE,
  L_ELBOW,
  L_FOOT,
...
function invalidFrame(side: 'left' | 'right', timestamp: number): FrameFeatures {
  return {
    valid: false,
    side,
    elbowAngle: NaN,
...
export function extractFrameFeatures(
  ...
  /*
   * mean_visibility must reproduce ml/src/features.py exactly, because the
   * shipped model was trained on vectors produced by that implementation.
   *
   * Python computes it as the mean of the two per-side mean visibilities:
   *     mean_vis = (side_score + other_score) / 2
   * where each score averages that side's six joints
   * (shoulder, elbow, wrist, hip, knee, ankle). Because both sides contribute
   * the same number of joints, that is identical to the mean over all twelve.
   *
   * Averaging a different, hand-picked set (e.g. both shoulders, both elbows
   * and both hips) is a plausible-looking but DIFFERENT quantity and made this
   * feature disagree with the Python reference by ~2.2e-3. The parity test in
   * tests/test_feature_parity.py caught it.
   */
  const meanVisibility = meanOf(lms, [
  ...
  if (torsoHeight < 1e-6) torsoHeight = 1e-6;
  ...
  if (ctx.prevNormalized) {
    ...
    for (const i of key) {
      ...
      if (prev && norm[i]) d.push(vlen(vsub(norm[i], prev)));
    }
...
function longestRun(ts: number[], mask: boolean[]): number {
  let best = 0;
  let start = -1;
  ...
  if (start >= 0) best = Math.max(best, ts[ts.length - 1] - ts[start]);
  return best;
}
 ...
 */
export function aggregateRepWindow(
  ...
  for (let i = 0; i < elbow.length; i++) {
    if (elbow[i] > elbow[peakIdx]) peakIdx = i;
    if (elbow[i] < elbow[troughIdx]) troughIdx = i;
  }
...
function meanOf(lms: Landmark[], idx: number[]): number {
  let s = 0;
  let n = 0;
  for (const i of idx) {
    if (lms[i]) {
...
function minOf(lms: Landmark[], idx: number[]): number {
  let m = Infinity;
  for (const i of idx) if (lms[i]) m = Math.min(m, lms[i].visibility);
